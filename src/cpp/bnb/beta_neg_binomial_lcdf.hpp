...
namespace bnb_gp_awgtr_model_namespace {
...
inline stan::return_type_t<T_size1, T_size2> beta_neg_binomial_lcdf(const T_n& n, 
                                                    const T_r& r,
                                                    const T_size1& alpha,
                                                    const T_size2& beta,
  ...
  for (size_t i = 0; i < max_size_seq_view; i++) {
    ...
    const T_partials_return P_i = 1 - C;
    ...
    if (!is_constant_all<T_r, T_size1, T_size2>::value) {
      // printf("n_dbl: %f, r_dbl: %f, alpha_dbl: %f, beta_dbl: %f, b_plus_n: %f, r_plus_n: %f, a_plus_r: %f\n", n_dbl, r_dbl, alpha_dbl, beta_dbl, b_plus_n, r_plus_n, a_plus_r);
